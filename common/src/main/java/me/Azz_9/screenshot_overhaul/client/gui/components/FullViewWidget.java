...
public class FullViewWidget extends AbstractWidget {
		...
		copyButton = Button.builder(
						...
						btn -> {
							if (currentScreenshot != null) {
								btn.setMessage(Component.translatable("screenshot_overhaul.copying"));
								CopyScreenshot.copyToClipboardWithToastError(currentScreenshot.file(), () -> {
									btn.setMessage(Component.translatable("screenshot_overhaul.copied"));
									copyResetAt = System.currentTimeMillis() + COPY_RESET_TIMER_MS;
								}, () -> btn.setMessage(Component.translatable("screenshot_overhaul.copy")));
							}
						})
	...
	protected void extractWidgetRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
		...
		if (copyResetAt != -1 && System.currentTimeMillis() >= copyResetAt) {
			copyButton.setMessage(Component.translatable("screenshot_overhaul.copy"));
			copyResetAt = -1;
		}
