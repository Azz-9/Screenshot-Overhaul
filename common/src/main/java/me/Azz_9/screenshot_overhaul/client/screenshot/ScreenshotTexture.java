...
public final class ScreenshotTexture implements AutoCloseable {
	...
	private ScreenshotTexture(@NonNull Path file, int maxSize) {
		this(file, () -> {
	...
	private ScreenshotTexture(@NonNull Path file, @NonNull NativeImage image, int maxSize) {
		this(file, () -> ImageScaler.downscale(image, maxSize));
	}

	private ScreenshotTexture(@NonNull Path file, @NonNull Supplier<NativeImage> imageSupplier) {
		this.file = file;
	...
	private ScreenshotTexture(@NonNull Path file) {
		this(file, 0);
	}

	public static @NonNull ScreenshotTexture loadSmallThumbnail(@NonNull Path file) {
		return new ScreenshotTexture(file, 128); // 128x72
	}

	public static @NonNull ScreenshotTexture loadThumbnail(@NonNull Path file) {
		return new ScreenshotTexture(file, 512); // 512x288
	}

	public static @NonNull ScreenshotTexture loadThumbnail(@NonNull Path file, @NonNull NativeImage image) {
		return new ScreenshotTexture(file, image, 512); //512x288
	}

	public static @NonNull ScreenshotTexture loadScreenshot(@NonNull Path file) {
	...
	public void whenReady(@NonNull BiConsumer<NativeImage, Throwable> consumer) {
