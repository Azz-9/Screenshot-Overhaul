...
public class ScreenshotReader extends ElementReader<Screenshot, ScreenshotRenderContext, ScreenshotRenderer> {
	...
	public ArrayList<RightClickOption> getRightClickOptions(Screenshot screenshot, IRightClickableElement target) {
		...
		options.add(new RightClickOption(Component.translatable("screenshot_overhaul.copy").getString(), 0, target) {
			@Override
		...
		options.add(new RightClickOption(Component.translatable("screenshot_overhaul.hide_from_worldmap").getString(), 1, target) {
			@Override
			public void onAction(Screen screen) {
